...
// Fetch returns the value specified by the path given,
...
// Example: val := sl.Fetch(virtualGuest, "Datacenter.Name")
func Fetch(s interface{}, path string, d ...interface{}) interface{} {
	var (
		val interface{}
		ok  bool
	)

	if val, ok = FetchOk(s, path); ok {
		return val
	}
...
// FetchOk returns the value specified by the path given,
// starting from the struct s.
// If at any point in the path the lookup falls short
// (i.e. a field is not found), or if the last field in the path is nil
// itself, a type-appropriate zero-value is returned.
...
//
// This is useful for getting values our of deeply nested structures
// Example: val, ok := sl.FetchOk(virtualGuest, "Datacenter.Name")
func FetchOk(s interface{}, path string) (interface{}, bool) {
	t := reflect.TypeOf(s)
	...
	return FetchOk(result, path[dotIndex+1:len(path)])
}
